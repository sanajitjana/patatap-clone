...
var paper = function (self, undefined) {
  ...
  var Item = Base.extend(
    ...
    },
    {
      ...
      rasterize: function (resolution, insert) {
        ...
        raster.transform(
          new Matrix().translate(topLeft.add(size.divide(2))).scale(1 / scale)
        );
        ...
        return {
          ...
          Rectangle: function () {
            var rect = Rectangle.readNamed(arguments, "rectangle"),
              radius = Size.min(
                Size.readNamed(arguments, "radius"),
      ...
      isCrossing: function () {
        ...
        function addOffsets(curve, end) {
          var v = curve.getValues(),
            ...
            count = roots.length,
            t =
              end && count > 1 ? roots[count - 1] : count > 0 ? roots[0] : 0.5;
    ...
    function importGradient(node, type) {
      ...
      if (radial) {
        ...
        destination = origin.add(
          getValue(node, "r", false, false, scaleToBounds),
          0
        );
      ...
      function onLoad(svg) {
        try {
          var node =
            typeof svg === "object"
              ? svg
              : new self.DOMParser().parseFromString(svg, "image/svg+xml");
          if (!node.nodeName) {
            node = null;
  ...
  Base.exports.PaperScript = function () {
    ...
    function execute(code, scope, options) {
      ...
      expose(
        { __$__: __$__, $__: $__, paper: scope, view: view, tool: tool },
        true
      );
      expose(scope);
